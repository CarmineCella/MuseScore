...
struct SoundTarget : public TargetI<T> {
    ...
    // samples are supposed to have 44100Hz sample rate
	void analyze_raw (const T* buffer, int samples) {
		TargetI<T>::segments.clear ();
		...
		for (unsigned i = 0; i < onsets.size (); ++i) {
            ...
            if (start > samples || end > samples) {
                throw std::runtime_error ("manually entered segment onset is beyond audio end");
            }
...
#warning DG: @Carmine: I have added a resample() function to the utilities (no dependency attached), I think Alessandro is testing it for the GUI. If you want you can call it here if the SR is not 44100.

		analyze_raw(buffer, samples);
