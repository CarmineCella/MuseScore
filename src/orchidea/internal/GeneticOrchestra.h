...
struct GeneticOrchestra : public OptimizerI<T> {
	...
	T search (OrchestrationModel<T>& model) {
		...
		for (unsigned i = 0; i < OptimizerI<T>::parameters->max_epochs; ++i) {
			...
			if (fit_count > MAX_EQUAL_EPOCHS || OptimizerI<T>::stop_flag) {
                for(int j = i; j < OptimizerI<T>::parameters->max_epochs; ++j) {
                    if (OptimizerI<T>::parameters->callback != nullptr) {
                        std::stringstream msg;
                        msg << "genetic search epoch " << i + 1;
                        OptimizerI<T>::parameters->callback->notifier (msg.str ().c_str (),
		...
		if (model.solutions.size ()) {
			std::vector<T> forecast (model.segment->features.size (), 0);
			ForecastPolicy<T>::compute(model.solutions.at (0), model.database, forecast,
				model.segment->features, OptimizerI<T>::parameters);
				history.push_front (forecast); // FIFO
		}
		return max_fit;
	}
private:
	...
	// evaluation --------------------------------------------------------------
	T compute_fitness (const std::vector<T>& values, const std::vector<T>& target) {
		T s = 0;
		#ifdef USE_DTW
		s = DTWDistance (target, values, OptimizerI<T>::parameters->positive_penalization,
			OptimizerI<T>::parameters->negative_penalization);
		#else
		for (unsigned i = 0; i < target.size(); ++i) {
	...
	T evaluate_individual (const Solution<T>& id, const std::vector<T>& target,
		...
		// fitness
		T s = compute_fitness(values, target);
		...
		T memory = 0;
		T forget = 1;
		for (unsigned i = 0; i < history.size () && forget > .0001; ++i) { // skip if small
			T h = compute_fitness (values, history.at (i));
		 	forget *= OptimizerI<T>::parameters->hysteresis; // decaying rate
			memory += h * forget;
		}
		return s + reg + memory;
	}
...
private:
	int best_epoch;
	std::deque<std::vector<T> > history;
};
