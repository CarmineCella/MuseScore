...
class Biquad {
  public:
	...
	Biquad (const float& sr, filterType type, const float& frequency,
			const float& q, const float& dbGain, bool qIsBandwidth) {
	...
	void reset (const float& sr, filterType type, const float& frequency,
				const float& q, const float& dbGain, bool qIsBandwidth) {
		...
		if (type >= PEAKING) {
			float const A		= pow (10.0, (dbGain / 40.0));
			float const omega	= 2. * temp_pi * frequency / sr;
			float const tsin	= sin (omega);
			float const tcos	= cos (omega);
			...
			if (qIsBandwidth) {
				alpha = tsin * sinh (log (2.) / 2. * q); // * q * omega / tsin);
			}
			else {
				alpha = tsin / (2. * q);
			}
