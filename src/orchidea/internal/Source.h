...
struct DB_entry {
	...
	typename std::deque<std::string> symbols;
};
...
void extract_symbols (DB_entry<T>& e) {
	...
	if (syms.size () == 4) e.symbols = syms; // instr technique note dynamics
		...
		for (unsigned i = 1; i < syms.size (); ++i) {
			if ((syms[i][0] == 'N' || syms[i][0] == 'n' ||
				syms[i][0] == 'A' || syms[i][0] == 'a' ||
				syms[i][0] == 'B' || syms[i][0] == 'b' ||
				syms[i][0] == 'C' || syms[i][0] == 'c' ||
				syms[i][0] == 'D' || syms[i][0] == 'd' ||
				syms[i][0] == 'E' || syms[i][0] == 'e' ||
				syms[i][0] == 'F' || syms[i][0] == 'f' ||
				syms[i][0] == 'G' || syms[i][0] == 'g') &&
				syms[i].size () <= 3) {
				pos = i;
				break;
			}
	...
	void load () {
		...
		for (unsigned i = 0; i < parameters->db_files.size (); ++i) {
			...
			while (!db.eof () ) {
				...
				if (results.size () < 1) {
					std::stringstream err;
					err << "invalid syntax in db at line " << lineno;
				...
				for (unsigned i = 1; i < results.size (); ++i) {
					...
					float f = atof (results.at (i).c_str ()); //token.c_str ());
