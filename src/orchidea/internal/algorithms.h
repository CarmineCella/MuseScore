...
// -----------------------------------------------------------------------------
// dynamic programming implementation of asymmetric dynamic time warping
template <typename T>
T DTWDistance (const std::vector<T>& s, const std::vector<T>& t, T positive_penalization,
    ...
    for (int i = 0; i < n; ++i) {
    	for (int j = 0; j < m; ++j) {
       		DTW[i][j] = std::numeric_limits<T>::max();
		}
	}
    DTW[0][0] = 0;
	...
	for (int i = 1; i < n; ++i) {
    	for (int j = 1; j < m; ++j) {
			T v = s[i] - t[j];
            T cost = v < 0 ? fabs (v) * negative_penalization : v * positive_penalization; // asymmetric
			//T cost = fabs (s[i] - t[j]);
	...
	for (unsigned i = 0; i < n; ++i) delete[] DTW[i];
	delete[] DTW;
	return r;
}
